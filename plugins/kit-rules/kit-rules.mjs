...
/**
 * The machine's own facts, generated by `kit-update.mjs --apply` beside the rules.
 *
 * It is a second file, not a section of the rules: the rules are one copy shared by every
 * machine and are content-hashed for convergence, while these facts are true of one machine
 * only. Missing is normal (a machine that has not applied yet), so its absence is silent.
 */
...
export function apply(ctx) {
  ...
  const factsPath = join(home, MACHINE_FACTS_FILE)
  ...
  ctx.effect(() => {
    const dispose = ctx.systemPrompt.section({
      ...
      text: () => {
        ...
        reportedPath = null
        // The machine's facts follow the rules, under the same framing, so an agent reads
        // what this machine IS in the same place it reads what it must DO — instead of
        // probing for a GPU every session.
        const facts = readRules(factsPath)
        return facts === null ? `${FRAMING}\n\n${rules}` : `${FRAMING}\n\n${rules}\n\n${facts}`
      },
    })
    return () => dispose()
  })
