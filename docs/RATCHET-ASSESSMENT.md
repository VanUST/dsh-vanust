...
A second independent breaker, given the corrected code, found **two more** — and the same
lesson twice over, because both were in the rule I had just repaired:

- `path_boundary` names its target in `zone` and `deny`, a **fourth** field I had not read
  at all, so a record declaring one zone could constrain a human-only zone.
- `pathIsGoverned` compared containment in **both** directions, so a target *broader* than
  the zone was accepted: a record declaring `plugins/demo/**` could carry a law over
  `plugins/**`, and then enforce on `plugins/other/**` — outside its authority by
  construction.

Both are fixed, and the second makes the rule's direction explicit: a target broader than
the zone reaches outside it and is refused, never accommodated. One further case was
reported and judged **out of scope**, with the reason stated rather than hidden: an orphan
whose `GENERATED BY ratchet compile` banner and `spec-hash` header are both stripped is no
longer identifiable as generated output, and that marker is the only thing distinguishing
it from a hand-written document the project may legitimately keep in that directory. The
...
invariants tested the case, not the class — twice.
