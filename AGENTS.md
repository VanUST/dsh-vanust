...
|---|---|---|
| `plugins/*.tgz` | built plugin tarballs (host + 2 clients) | every shipped plugin change — via `scripts/rebuild-plugins.sh`, then **commit** |
| `profile/cordis.patch.yml` | canonical profile patch: host row + two `dsh.client` rows | plugin ids/names change, or per-machine config is added (e.g. Windows `openCommand: explorer`) |
